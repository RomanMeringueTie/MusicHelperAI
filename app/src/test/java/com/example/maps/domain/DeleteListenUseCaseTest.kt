...
class DeleteListenUseCaseTest {

    private lateinit var useCase: DeleteListenUseCase
    private lateinit var mockRepository: ListensRepository

    @Before
    fun setup() {
        mockRepository = mockk(relaxed = true)
        useCase = DeleteListenUseCaseImpl(mockRepository)
    }

    @Test
    fun `invoke calls repository delete with correct listen`() = runTest {
        // Given
        ...
        // Then
        coVerify { mockRepository.delete(listen) }
    }

    @Test
    fun `invoke handles repository delete successfully`() = runTest {
        // Given
        val listen = ListenFull(
            title = "Test Track",
            artist = "Test Artist",
